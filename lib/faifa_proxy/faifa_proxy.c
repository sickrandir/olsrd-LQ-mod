...
int send_6048(u_int8_t *frame_buf, int frame_len, int cursor) {
	u_int8_t *frame_ptr = frame_buf;
	frame_ptr += cursor;
	frame_len = frame_ptr - (u_int8_t *) frame_buf;
...
void get_PLC_mac() {
	...
	c = init_frame(frame_buf, frame_len, 0x6048);
	s = send_6048(frame_buf, frame_len, c);
	receive_frame();
	printf("Faifa proxy: My PLC MAC address: %02x:%02x:%02x:%02x:%02x:%02x\n", plc_mac[0], plc_mac[1], plc_mac[2], plc_mac[3], plc_mac[4], plc_mac[5]);
}
...
int serialize_own_plc_mac(unsigned char *buff) {
	buff[0] = plc_mac[0];
	buff[1] = plc_mac[1];
	buff[2] = plc_mac[2];
	buff[3] = plc_mac[3];
...
int server(int client_socket) {
	...
	if (msg == 'm') {
		unsigned char mac[6];
		serialize_own_plc_mac(&mac);
		int j;
		send(client_socket, mac, 6, 0);
		return 0;
	}
	if (msg == 'p') {
...
int main(int argc, char **argv) {
	...
	do {
		...
		if (retval == -1)
			...
			printf("faifa_proxy: Prima di accept.\n");
			...
			printf("faifa_proxy: Non ho ricevuto nulla nell'arco di 4 secondi.\n");
	...
	faifa_printf(out_stream, "faifa_proxy: Ricevuto. Chiudo\n");
	faifa_close(faifa);
	faifa_free(faifa);
	return 0;
}
